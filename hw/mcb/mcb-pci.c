...
#define N_MODULES ((CHAMELEON_TABLE_SIZE - sizeof(ChameleonFPGAHeader))/sizeof(ChameleonDeviceDescriptor))
...
static uint64_t mpci_chamtbl_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    } else if (addr >= sizeof(ChameleonFPGAHeader) && addr < CHAMELEON_TABLE_SIZE) {
...
static void mcb_pci_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    memory_region_init(&bus->mmio_region, OBJECT(s), "mcb-pci.mmio", 2048 * 1024);
    ...
    pci_register_bar(&s->dev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY, &bus->mmio_region);
